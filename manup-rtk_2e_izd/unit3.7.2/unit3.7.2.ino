...
void loop()//функция движения манипулятора в диапазоне +-100 тиков
{
 ...
 get_data_load();//функция считывания нагрузки на сервоприводах
}

void get_data_load()//функция считывания нагрузки на сервоприводах
{
 int32_t get_present_load1 = 0;//переменная для записи текущей нагрузки
 dxl_wb.itemRead(1, "Present_Load", &get_present_load1);//считывание нагрузки
 delay(10);
 Serial.println("Текущая нагрузка на сервоприводе 1 = " + String(get_present_load1));

  int32_t get_present_load2 = 0;//переменная для записи текущей нагрузки
 dxl_wb.itemRead(2, "Present_Load", &get_present_load2);//считывание нагрузки 
 delay(10);
 Serial.println("Текущая нагрузка на сервоприводе 2 = " + String(get_present_load2));

  int32_t get_present_load3 = 0;//переменная для записи текущей нагрузки
 dxl_wb.itemRead(3, "Present_Load", &get_present_load3);//считывание нагрузки 
 delay(10);
 Serial.println("Текущая нагрузка на сервоприводе 3 = " + String(get_present_load3));

  int32_t get_present_load4 = 0;//переменная для записи текущей нагрузки
 dxl_wb.itemRead(4, "Present_Load", &get_present_load4);//считывание нагрузки 
 delay(10);
 Serial.println("Текущая нагрузка на сервоприводе 4 = " + String(get_present_load4));

  int32_t get_present_load5 = 0;//переменная для записи текущей нагрузки
 dxl_wb.itemRead(5, "Present_Load", &get_present_load5);//считывание нагрузки 
 delay(10);
 Serial.println("Текущая нагрузка на сервоприводе 5 = " + String(get_present_load5));
}