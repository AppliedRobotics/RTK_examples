...
void loop()//функция движения манипулятора в диапазоне +-100 тиков
{
 ...
 get_data_temp();//функция считывания температуры сервоприводов
}

void get_data_temp()//функция считывания температуры сервоприводов
{
  int32_t get_present_temp1 = 0;//переменная для записи текущей температуры
 dxl_wb.itemRead(1, "Present_Temperature", &get_present_temp1);//считывание температуры
 delay(10);
 Serial.println("Текущая температура на сервоприводе 1 = " + String(get_present_temp1));

  int32_t get_present_temp2 = 0;//переменная для записи текущей температуры
 dxl_wb.itemRead(2, "Present_Temperature", &get_present_temp2);//считывание температуры
 delay(10);
 Serial.println("Текущая температура на сервоприводе 2 = " + String(get_present_temp2));

  int32_t get_present_temp3 = 0;//переменная для записи текущей температуры
 dxl_wb.itemRead(3, "Present_Temperature", &get_present_temp3);//считывание температуры 
 delay(10);
 Serial.println("Текущая температура на сервоприводе 3 = " + String(get_present_temp3));

  int32_t get_present_temp4 = 0;//переменная для записи текущей температуры
 dxl_wb.itemRead(4, "Present_Temperature", &get_present_temp4);//считывание температуры 
 delay(10);
 Serial.println("Текущая температура на сервоприводе 4 = " + String(get_present_temp4));

  int32_t get_present_temp5 = 0;//переменная для записи текущей температуры
 dxl_wb.itemRead(5, "Present_Temperature", &get_present_temp5);//считывание температуры 
 delay(10);
 Serial.println("Текущая температура на сервоприводе 5 = " + String(get_present_temp5));
}
